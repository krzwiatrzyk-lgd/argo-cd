...
func NewCommand() *cobra.Command {
	...
	command := cobra.Command{
		...
		RunE: func(c *cobra.Command, _ []string) error {
			...
			appsetReconciler.ProgressiveSyncManager.SettleWindow = progressiveSyncSettleWindow
	...
	command.Flags().DurationVar(&progressiveSyncSettleWindow, "progressive-sync-settle-window", env.ParseDurationFromEnv("ARGOCD_APPLICATIONSET_CONTROLLER_PROGRESSIVE_SYNC_SETTLE_WINDOW", 0, 0, time.Minute*5), "Quiet period to wait after a progressive sync first observes a change before promoting any Application to Pending. Gives Applications that have not refreshed yet time to register the same change, so a RollingSync step is not released against a revision an earlier step has not seen. 0 disables the wait. Requires --enable-progressive-syncs.")
