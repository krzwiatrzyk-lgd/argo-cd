...
func (r *ApplicationSetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (result ctrl.Result, err error) {
	...
	// progressiveSyncRequeue is how long progressive sync wants to wait before deciding again, for
	// example while it withholds the rollout decision. Zero when it needs no requeue of its own.
	...
	// Folded in last so it can only ever bring the requeue forward: the validation error fallback
	// above still decides whether a requeue happens at all, and every other schedule keeps its
	// meaning.
